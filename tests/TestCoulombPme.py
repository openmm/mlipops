...
def test_batch(device, max_multipole):
    ...
    cutoff = 0.4
    neighbor_list = NeighborList(cutoff, device=device)
    pme = CoulombPME(neighbor_list, None, 14, 16, 15, 5, 5.0, 138.935, max_multipole=max_multipole)
    energy = pme(positions, charges, box_vectors, dipoles=dipoles, batch=batch)
    for i in range(num_systems):
        mask = batch == i
        energy1 = energy[i]
        ...
        energy2 = pme(pos, charges[mask], box, dipoles=dipoles[mask])
