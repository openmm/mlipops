...
class CoulombPME(torch.nn.Module):
    ...
    def forward(self, positions: torch.Tensor, charges: torch.Tensor, box_vectors: torch.Tensor, include_direct: bool = True,
                include_reciprocal: bool = True, dipoles: torch.Tensor | None = None,
    ...
    def compute_field(self, field_positions: torch.Tensor, positions: torch.Tensor, charges: torch.Tensor,
        ...
        if include_reciprocal:
            ...
            ti, _, data, ddata = compute_spline_coefficients(self, field_positions, recip_box_vectors, grid_size_tensor, None)
...
def compute_spline_coefficients(pme: CoulombPME, positions: torch.Tensor, recip_box_vectors: torch.Tensor,
    ...
    if batch is None:
        ...
        t = torch.einsum('ij,ijk->ik', positions, recip_box_vectors[batch])
...
def reciprocal_forward(pme: CoulombPME, positions: torch.Tensor, charges: torch.Tensor, dipoles: torch.Tensor | None,
    ...
    ti, dr, data, ddata = compute_spline_coefficients(pme, positions, recip_box_vectors, grid_size_tensor, batch)
    order = pme.order
    num_particles = positions.shape[0]
    ...
    recip_grid = torch.fft.rfftn(grid, dim=(-3, -2, -1))
    if batch is None:
        scale_factor = torch.pi*box_vectors.diag().prod()
    else:
        scale_factor = torch.pi*torch.diagonal(box_vectors, dim1=1, dim2=2).prod(dim=1).view(-1, 1, 1, 1)
    recip_exp_factor = (torch.pi/pme.alpha)**2
    kx = torch.arange(recip_grid.shape[-3], device=recip_grid.device)
    ky = torch.arange(recip_grid.shape[-2], device=recip_grid.device)
    kz = torch.arange(recip_grid.shape[-1], device=recip_grid.device)
    batch_dim = tuple() if batch is None else (1,)
    mx = (kx - (kx >= (pme.gridx+1)/2)*pme.gridx).view(batch_dim+(-1,1,1)).expand(recip_grid.shape)
    my = (ky - (ky >= (pme.gridy+1)/2)*pme.gridy).view(batch_dim+(1,-1,1)).expand(recip_grid.shape)
    mz = (kz - (kz >= (pme.gridz+1)/2)*pme.gridz).view(batch_dim+(1,1,-1)).expand(recip_grid.shape)

    def recip_slice(i, j):
        if batch is None:
            return recip_box_vectors[i, j]
        return recip_box_vectors[:, i, j].view(-1, 1, 1, 1)

    mhx = mx*recip_slice(0, 0)
    mhy = mx*recip_slice(1, 0) + my*recip_slice(1, 1)
    mhz = mx*recip_slice(2, 0) + my*recip_slice(2, 1) + mz*recip_slice(2, 2)
    m2 = mhx*mhx + mhy*mhy + mhz*mhz
    moduli = pme.xmoduli[kx].view(batch_dim+(-1,1,1)) * pme.ymoduli[ky].view(batch_dim+(1,-1,1)) * pme.zmoduli[kz].view(batch_dim+(1,1,-1))
...
class ReciprocalFunction(torch.autograd.Function):
    ...
    def forward(ctx, pme: CoulombPME, positions: torch.Tensor, charges: torch.Tensor, dipoles: torch.Tensor | None,
        ...
        energy = torch.sum(scale*eterm*(recip_grid.real*recip_grid.real + recip_grid.imag*recip_grid.imag), dim=(-3,-2,-1))
        ctx.save_for_backward(positions, charges, dipoles, transformed_dipoles, recip_box_vectors, ti, dr, data, ddata,
                              recip_grid*eterm, batch)
        ctx.pme = pme
        return 0.5*energy

    @staticmethod
    def backward(ctx, *grad_outputs: torch.Tensor):
        positions, charges, dipoles, transformed_dipoles, recip_box_vectors, ti, dr, data, ddata, recip_grid, batch = ctx.saved_tensors
        pme = ctx.pme
        device = pme.xmoduli.device
        grid_size = (pme.gridx, pme.gridy, pme.gridz)
        grid_size_tensor = torch.tensor(grid_size, device=device)
        ...
        for iy in range(order):
            ...
            for iz in range(order):
                ...
                if batch is None:
                    ...
                    grid.index_put_((batch, xindex, yindex, zindex), values, accumulate=True)
    return grid
        ...
        for iy in range(order):
            ...
            for iz in range(order):
                ...
                if batch is None:
                    ...
                    g = grid[batch, xindex, yindex, zindex]
                if charge_deriv is not None:
